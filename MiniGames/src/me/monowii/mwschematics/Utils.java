...
public class Utils {
	...
	public static boolean copySchematic(Location loc, File schematicFile, boolean dontcopyAir, boolean removeEntities) {
		...
		if (schematic == null) {
			System.err.println("Unknown schematic format " + schematicFile);
			return false;
		}
		try {
			...
			if (removeEntities) {
				final Location p1 = new Location(w, origin.getX() + clipboard.getOffset().getX(), origin.getY()
						+ clipboard.getOffset().getY(), origin.getZ() + clipboard.getOffset().getZ());
				final Location p2 = new Location(w, p1.getX() + clipboard.getWidth(),
						p1.getY() + clipboard.getHeight(), p1.getZ() + clipboard.getLength());

				removeEntities(p1, p2);
			}
			return true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static void removeEntities(Location p1, Location p2) {
		...
		for (int x = Math.min(c1.getX(), c2.getX()); x <= Math.max(c1.getX(), c2.getX()); x++)
			for (int z = Math.min(c1.getZ(), c2.getZ()); z <= Math.max(c1.getZ(), c2.getZ()); z++)
				for (Entity e : p1.getWorld().getChunkAt(x, z).getEntities())
					if ((e.getLocation().getBlockX() >= xMin || e.getLocation().getBlockX() <= xMax)
							&& (e.getLocation().getBlockY() >= yMin || e.getLocation().getBlockY() <= yMax)
							&& (e.getLocation().getBlockZ() >= zMin || e.getLocation().getBlockZ() <= zMax))
						if (e instanceof Projectile || e instanceof Boat || e instanceof Item
								|| e instanceof FallingBlock || e instanceof Minecart || e instanceof Hanging
								|| e instanceof TNTPrimed || e instanceof ExperienceOrb) e.remove();
	}

	public static boolean argsContain(String[] args, String arg) {
		for (int i = 0; i < args.length; i++)
			if (args[i].equalsIgnoreCase(arg) || args[i].startsWith(arg)) return true;
